...
##########################################
'''
this class is used to report the result of Simulation 1, Section 3.1:
...
class simul_plot:
    ...
    def __init__(self, sample_size, n_dim, n_info, n_repeat, num_rep, step_size, rnd_seed, plot_on):
    ...
    #the histogram of number of variables selected : solar vs competitors
        ...
        if self.plot_on == True:
            f11.savefig("./figure/sparsity_plot_n_"+str(self.sample_size)+"_vs_CVcd.pdf", bbox_inches='tight')
        ...
        #output it into pdf
        if self.plot_on == True:
            f1.savefig("./figure/sparsity_plot_n_"+str(self.sample_size)+"_vs_CVlars.pdf", bbox_inches='tight')
    ...
    #histogram plot of c* of solar in 200 repeats
        ...
        if self.plot_on == True:
            f2.savefig("./figure/q-values_plot_n_"+str(self.sample_size)+".pdf", bbox_inches='tight')
    ...
    #bar plot: probability of solar/cv-lars-lasso/cv-cd selecting each variable (all 5 informative variables; redundant variables (of top 35 probability))
        ...
        if self.plot_on == True:
            f4.savefig("./figure/acc_plot_top" + str(num_var_to_plot) + "_n_" + str(self.sample_size) + "_" + str(print_true) + "_solar.pdf", bbox_inches='tight')
        ...
        #output it as pdf file
        if self.plot_on == True:
            f44.savefig("./figure/acc_plot_top" + str(num_var_to_plot) + "_n_" + str(self.sample_size) + "_" + str(print_true) + "_lars.pdf", bbox_inches='tight')
        ...
        #output it as pdf file
        if self.plot_on == True:
            f444.savefig("./figure/acc_plot_top" + str(num_var_to_plot) + "_n_" + str(self.sample_size) + "_" + str(print_true) + "_cd.pdf", bbox_inches='tight')
    ...
    #since we only boxplot olar regression coefficients (of top 15 means), we do the ranking again
        ...
        if self.plot_on == True:
            f3.savefig("./figure/solar_vari_plot_top"+str(num_var_to_plot)+"_n_"+str(self.sample_size)+".pdf", bbox_inches='tight')
