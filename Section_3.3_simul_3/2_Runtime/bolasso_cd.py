...
####################################################################################
# the class of bolasso using coordinate descent and built-in parallel archetecture #
####################################################################################

'''
this class is used to demonstrate the performance of bolasso using the sklearn built-in parallel archetecture

please note that this package is almost identical with "bolasso_parallel.py". The only difference is that 
    1. in this file, we don't have to manually set a parallel scheme. We use the default sklearn parallel
    2. in "bolasso_parallel.py", we design a high-level parallel scheme ourselves.

Check this before you run the code:
Plz check if you have 'scikit-learn', 'numpy', 'joblib', 'matplotlib' and 'tqdm' installed. If not,
...
class bolasso:
    ...
    def __init__(self, X, Y, n_repeat_bolasso):
    ...
    def fit(self):
        ...
        for j in range(self.n_repeat_bolasso):

            # 2a. randomly choose a bootstrap set of sample points (whose index is 'index_subsample'); 
            ...
            # 2b. based on the index "index_subsample", take the corresponding observations of X as "X_subample"
            X_subsample = self.X[index_subsample]
            # 2c. based on the index "index_subsample", take the corresponding observations of Y as "y_subample"
            y_subsample = self.y[index_subsample]
            # 2d. change dimension for the Sklearn lasso package.
            y_subsample.shape = (y_subsample.shape[0],)

            # 2e. given a subsample, compute lasso and output the active set. 
            ...
            active = np.nonzero(trial_1.coef_)

            # 2f. based on the active set of lasso, we compute qhat^k as the binary string of whether each variable is selected by lasso on subsample K
            # we generate 'qhat_k' as a row of zeros;
            qhat_k = np.zeros((1, self.n_dim))
            # 2f(2). if a variable (the ith column in matrix X) is selected by lasso, we change the ith value of qhat_k (the ith column) as 1
            for i in active:
