...
class solar:
    ...
    def __init__(self, X, Y, n_repeat, step_size, rnd=0, lasso=True):
    ...
    def fit(self):
        ...
        #to make parallel computing replicable, set parent random seeds
        np.random.seed(self.rnd)
        #based on the parent random seeds, pawn off 10 child SeedSequences to pass to child processes.
        seeds = np.random.randint(2e8, size=self.n_repeat)
        ...
        def loop_fun(self, i, seeds, qhat_k_container):
            ...
            # 2e. given a subsample, compute q_hat^k (the solution path) using lars
    ...
    # return the list of all Q(c), for all c from (start from q_start and end at q_end)
