...
##########################################
'''
this class is used for plotting the result of Simulation 1, Section 4:
...
Inputs:
    ...
    6. num_rep     : the number of repeatitions in Simulation 2 (200 in paper)
...
class simul_plot:
    ...
    def __init__(self, sample_size, n_dim, n_info, n_repeat, num_rep, step_size, rnd_seed):
    ...
    #compute 200 repeats of solar vs cv-lars-lasso and cv-cd
        ...
        ##for reproduction, fix the random seed using 'numpy'
        ...
        return opt_c_stack, Q_opt_c_stack, la_array_stack, la_var_stack, solar_coef_stack, cd_array_stack, cd_var_stack
    ...
    #the histogram of number of variables selected : solar vs competitors
        ...
        #output it into pdf
        f11.savefig("sparsity_plot_n_"+str(self.sample_size)+"_p_"+str(self.n_dim)+"_vs_CVcd.pdf", bbox_inches='tight')
        ...
        #legend of solar and cv-lars-lasso
        plt.legend(loc=9, bbox_to_anchor=(0.5, -0.2), borderaxespad=0., ncol=3, shadow=True)
        plt.xlabel('number of variables selected', fontsize=16)
        plt.xlim(3,50)
        plt.ylim(0,0.6)
        ...
        #output it into pdf
        f1.savefig("sparsity_plot_n_"+str(self.sample_size)+"_p_"+str(self.n_dim)+"_vs_CVlars.pdf", bbox_inches='tight')

        #count how many repeats solar select 5 variables
        print("in " + str(np.count_nonzero(solar_len_array- 5)) + " out of 200 solar pick more/less than 5 variables")
        #count how many repeats CV-lars-lasso select 5 variables
        print("in " + str(np.count_nonzero(np.array(la_array_stack)  - 5)) + " out of 200 CV-lars-lasso  pick more/less than 5 variables")
        #count how many repeats CV-cd select 5 variables
        print("in " + str(np.count_nonzero(np.array(cd_array_stack)  - 5)) + " out of 200 CV-cd pick more/less than 5 variables")


    def q_hist(self, opt_c_stack):
    #histogram plot of c* of solar in 200 repeats
        ...
        f2.savefig("q-values_plot_n_"+str(self.sample_size)+"_large_p.pdf", bbox_inches='tight')
    ...
    #bar plot: probability of solar/cv-lars-lasso/cv-cd selecting each variable (all 5 informative variables; redundant variables (of top 35 probability))
        ...
        f4.savefig("acc_plot_top" + str(num_var_to_plot) + "_n_" + str(self.sample_size)+ "_p_" + str(self.n_dim) + "_" + str(print_true) + "_solar.pdf", bbox_inches='tight')


        #bar plot for cv-lars-lasso
        f44 = plt.figure()

        # The big plot
        ax00 = f44.add_subplot(111)

        # Turn off axis lines and ticks of the big plot
        ...
        #output it as pdf file
        f44.savefig("acc_plot_top" + str(num_var_to_plot) + "_n_" + str(self.sample_size) +  "_p_" + str(self.n_dim) + "_" + str(print_true) + "_lars.pdf", bbox_inches='tight')
        ...
        #the bar plot of the probability of cv-cd selecting each variable
        ...
        #output it as pdf file
        f444.savefig("acc_plot_top" + str(num_var_to_plot) + "_n_" + str(self.sample_size) +  "_p_" + str(self.n_dim) + "_" + str(print_true) + "_cd.pdf", bbox_inches='tight')
    ...
    #since we only boxplot olar regression coefficients (of top 15 means), we do the ranking again
        ...
        print('the number of non-zeros in the boxplot of beta_0: ', 200-len(np.where(bl_coef_stack[0,:] == 0)[0]))
        #count in how many repeats solar estimate the regression coefficient of x_1 as 1
        print('the number of non-zeros in the boxplot of beta_1: ', 200-len(np.where(bl_coef_stack[1,:] == 0)[0]))
        #count in how many repeats solar estimate the regression coefficient of x_2 as 2
        print('the number of non-zeros in the boxplot of beta_2: ', 200-len(np.where(bl_coef_stack[2,:] == 0)[0]))
        #count in how many repeats solar estimate the regression coefficient of x_3 as 3
        print('the number of non-zeros in the boxplot of beta_3: ', 200-len(np.where(bl_coef_stack[3,:] == 0)[0]))
        #count in how many repeats solar estimate the regression coefficient of x_4 as 4
        print('the number of non-zeros in the boxplot of beta_4: ', 200-len(np.where(bl_coef_stack[4,:] == 0)[0]))

        #output it as pdf file
        f3.savefig("solar_vari_plot_top"+str(num_var_to_plot)+"_n_"+str(self.sample_size)+  "_p_" + str(self.n_dim) + ".pdf", bbox_inches='tight')
